...
import { loadOrganisationSecteur } from '../../../store/actions/OrganisationSecteurs/OrganisationSecteursActions';
...
const OrganisationSecteurs = () =>{
    //constants defined
	const [contents, setContents] = useState(tableList);
    let  dispatch = useDispatch();
    const {listorganisationsecteur} = useSelector(state=>state.organisationsecteur);
    ...
    console.log("loadorganisation",listorganisationsecteur);
			...
			<div className="col-12">
				...
				<div className="card">
					<div className="card-header">
						<h4 className="card-title">listes des secteurs</h4>
							...
							<div id="example_wrapper" className="dataTables_wrapper">
								<form onSubmit={handleEditFormSubmit}>
									<table id="example" className="display w-100 dataTable">
										...
										<tbody>
											{listorganisationsecteur && listorganisationsecteur.map((organisationsecteur)=>(
												<>
													{editContentId === organisationsecteur.id ? 
														( 
														...
														( 
															<tr>
																...
																<td>
                                                                    <span>{organisationsecteur.responsable.name} {organisationsecteur.responsable.firstname}</span>
                                                                </td>

																<td>{organisationsecteur.num_secteur}</td>
																{organisationsecteur.num_gfs.map(gf=>{
                                                                    return  <Fragment key={gf.id}>
                                                                                <td>{gf.bloc}</td>
                                                                                <td>{gf.num_gf}</td>
                                                                                <td>{gf.name}</td>
																...
																<td>
																	<div className="d-flex">
																		<Link  className="btn btn-secondary	 shadow btn-xs sharp me-2"
																			onClick={(event) => handleEditClick(event, organisationsecteur)}
																		>
																			<i className="fas fa-pen"></i>
																		</Link>
																		<Link  className="btn btn-danger shadow btn-xs sharp" 
																			onClick={()=>handleDeleteClick(organisationsecteur.id)}
																		> 
...
export default OrganisationSecteurs;