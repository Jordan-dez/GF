...
import { loadSuiviTelephonique } from '../../../store/actions/SuiviTelephonique/SuiviTelephoniqueActions';
...
const SuiviTelephonique = () =>{

    let dispatch= useDispatch();
    const {suivitelephoniques}= useSelector(state=>state.suivitelephoniques)

    useEffect(()=>{
        dispatch(loadSuiviTelephonique())

    },[])
    console.log("suivi tele",suivitelephoniques);
	...
	return(
		<>
			<PageTitle activeMenu="liste de suivi téléphonique" motherMenu="Suivi Téléphonique" />
								...
								<form onSubmit={handleEditFormSubmit}>
									<table id="example" className="display w-100 dataTable">
										<thead>
											<tr>
												<th></th>
												<th>personne appelée</th>
												<th>commentaires</th>
												<th>appel réalisé par</th>
												<th>created_at</th>
												<th>created_by</th>
												<th>created_by_id</th>
												<th>date_appel</th>
										...
										<tbody>
											{suivitelephoniques && suivitelephoniques.map((suivitelephonique)=>(
												<>
													{editContentId === suivitelephonique.id ? 
														( 
															<Editable editFormData={editFormData} 
																handleEditFormChange={handleEditFormChange} handleCancelClick={handleCancelClick} key={suivitelephonique.id}/> 
																) : 
														( 
															<tr>
																<td><img className="rounded-circle" width="35" src={pic1} alt="" /></td>
																<td>{suivitelephonique.nom.lastname}&nbsp;{suivitelephonique.nom.lastname}</td>
																<td>{suivitelephonique.commentaires}</td>
																<td>{suivitelephonique.create_by.name}</td>
																<td>{suivitelephonique.created_by}</td>
																<td><strong>{suivitelephonique.created_by_id}</strong></td>
																<td><strong>{suivitelephonique.date_appel}</strong></td>
																<td>
																	<div className="d-flex">
																		<Link  className="btn btn-secondary	 shadow btn-xs sharp me-2"
																			onClick={(event) => handleEditClick(event, suivitelephonique)}
																		>
																			<i className="fas fa-pen"></i>
																		</Link>
																		<Link  className="btn btn-danger shadow btn-xs sharp" 
																			onClick={()=>handleDeleteClick(suivitelephonique.id)}
																		> 
...
export default SuiviTelephonique;