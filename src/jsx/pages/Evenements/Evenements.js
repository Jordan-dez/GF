...
import { deleteEvenement, loadEvenement } from '../../../store/actions/Evenements/EvenementActions';
const tableList = [
	{
...
const Evenement = () =>{
    ...
    //selector
    const {listevenement}=useSelector(state=>state.evenements)
	// delete data  
    const handleDeleteClick = (evenementId) => {
        // const newContents = [...contents];    
        // const index = contents.findIndex((content)=> content.id === contentId);
        // newContents.splice(index, 1);
        // setContents(newContents);
        dispatch(deleteEvenement(evenementId));
    }
    ...
    console.log("evenement loaded",listevenement)
	
	return(
		<>
			<PageTitle activeMenu="Table" motherMenu="evenement" />
			<div className="col-12">
				...
				<div className="card">
					<div className="card-header">
						<h4 className="card-title">liste des évèmenets</h4>
							...
							<div id="example_wrapper" className="dataTables_wrapper">
								<form onSubmit={handleEditFormSubmit}>
									<table id="example" className="display w-100 dataTable">
										...
										<tbody>
											{listevenement && listevenement.map((evenement)=>(
												<>
													{editContentId === evenement.id ? 
														( 
														...
														( 
															<tr>
																...
																<td><strong>{evenement.date_fin}</strong></td>
																
																<td>
																	<div className="d-flex">
																		<Link  className="btn btn-secondary	 shadow btn-xs sharp me-2"
																			onClick={(event) => handleEditClick(event, evenement)}
																		>
																			<i className="fas fa-pen"></i>
																		</Link>
																		<Link  className="btn btn-danger shadow btn-xs sharp" 
																			onClick={()=>handleDeleteClick( parseInt(evenement.id) )}
																		> 
